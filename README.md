...
- 目标指定带有 `1` 秒冷却，避免高频攻击反复重设目标导致飞剑状态异常
...
- 飞行满 `60` 秒后会实体化为 `ThrownTrident`，像箭或三叉戟一样掉落并插在地上，等待玩家自行回收
...
- 客户端根据服务端同步的位置和速度快照做平滑跟随与朝向缓动，减轻飞行过程中的模型抖动
- 万剑归宗飞剑会按飞行方向调整姿态，并补一个 `45` 度模型朝向修正，让剑尖朝前
- 玩家已有万剑归宗飞剑在场时，会隐藏主手中对应的剑显示，避免“已发射但手上还拿着”的表现
...
- 当前目标设定方式是：玩家在已有飞剑存在时，近战命中实体或玩家发射的投射物命中实体后，将其登记为该玩家飞剑池的当前目标；目标指定存在 `1` 秒冷却
...
- 当前在锁定追踪、命中回弹和回群阶段会暂停寿命计时；寿命结束时才会把虚拟飞剑转换为 `ThrownTrident` 并掉落在世界中，由玩家自行拾取
