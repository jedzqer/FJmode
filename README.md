...
- 可同时存在多把飞剑
- 虚拟飞剑会在玩家上方较大范围内环绕飞行，并保留群飞轨迹
- 玩家在已有飞剑存在时，用附魔剑左键命中实体后，该实体会被设为当前追踪目标
- 追踪目标时飞剑会快速聚拢并扑向目标
...
- 当前目标设定方式是：玩家在已有飞剑存在时，使用附魔剑左键命中一个实体后，将其登记为该玩家飞剑池的当前目标
- 当前伤害规则是：参与本次命中的所有飞剑，逐把统计剑本体攻击力与附魔伤害修正，再将总和除以 `3`
- 当前发射时会直接取走主手中的真实剑；寿命结束时会把虚拟飞剑转换为 `ThrownTrident` 并掉落在世界中，由玩家自行拾取
